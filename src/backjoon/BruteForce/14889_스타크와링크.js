...
const players = Array(+N)
	.fill(0)
	.map((_, i) => i + 1);
const doubleArray = array.map((a) => a.split(' '));
function solution() {
	let min = 100;
	...
	function BT(numbers, currentIndex) {
		if (numbers.length === +N / 2) {
			const start = numbers;
			const link = players.filter((p) => !start.some((s) => s == p));
		...
		for (let j = currentIndex; j < +N; j++) {
			const number = players[j];
			BT([...numbers, number], j + 1);
		}
	}

	function getScore(team) {
		let sum = 0;
		function BT(teamMembers, currentIndex) {
			if (teamMembers.length === 2) {
				const member_i = +teamMembers[0] - 1;
				const member_j = +teamMembers[1] - 1;

				const score =
					+doubleArray[member_i][member_j] + +doubleArray[member_j][member_i];

				sum += score;
				return;
			}

			for (let i = currentIndex; i < +N / 2; i++) {
				const teamMember = team[i];
				BT([...teamMembers, teamMember], i + 1);
			}
		}
